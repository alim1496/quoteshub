...
class HomeQuotesAdapter(val context:Context) : PagedListAdapter<HomeQuote, RecyclerView.ViewHolder> (DIFF_CALLBACK) {
    ...
    companion object {
        private var DIFF_CALLBACK = object : DiffUtil.ItemCallback<HomeQuote>() {
            override fun areItemsTheSame(oldItem: HomeQuote, newItem: HomeQuote): Boolean {
                return oldItem.id == newItem.id
            }

            override fun areContentsTheSame(oldItem: HomeQuote, newItem: HomeQuote): Boolean {
    ...
    class QuoteViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bindView(homeQuote: HomeQuote, context: Context) {
            ...
            val quote = Quote(homeQuote.id, homeQuote.title, Source(1, homeQuote.source), false, "", "", homeQuote.featured)
            ...
            title.text = homeQuote.title
            source.text = homeQuote.source
        }
    }
}